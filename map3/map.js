window.addEventListener("DOMContentLoaded", async (_evt) => {
  const data = await d3.json(
    "//cdn.billmill.org/static/reassess2025/property_values.geojson",
  );
  map(data);
});
...
function map(data) {
  data.features = data.features.filter(
    (d) => d.properties?.y2025?.total && d.properties?.y2021?.total,
  );
  data.features.forEach(
    (d) =>
      (d.properties.diff =
        d.properties.y2025.total / d.properties.y2021.total - 1),
  );

  const scale = d3
    .scaleSequential(
      [
        d3.quantile(data.features, 0.05, (d) => d.properties.diff),
        d3.quantile(data.features, 0.95, (d) => d.properties.diff),
      ],
  ...
  const pointLayer = L.geoJSON(data, {
    pointToLayer: function (pt, latlng) {
      const color = scale(pt.properties.diff);
      ...
      marker.on("mouseover", () => {
        ...
        document.getElementById("value2025").innerText = dol(
          pt.properties.y2025.total,
        );
        document.getElementById("diff").innerText = pct(pt.properties.diff);
      });
      return marker;
    },
  }).addTo(map);
